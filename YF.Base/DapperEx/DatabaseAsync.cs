...
namespace Dapper
{
    public abstract partial class Database<TDatabase> : IDisposable where TDatabase : Database<TDatabase>, new()
    {
        public partial class Table<T, TId>
        {
            ...
            /// <returns></returns>
            public virtual async Task<int?> InsertAsync(dynamic data,bool autoId=true)
            {
                var o = (object)data;
                ...
                return await database.ExecuteAsync(sql, o);
            }

            public virtual async Task<int?> InsertReturnIdAsync(dynamic data, bool autoId = true)
            {
                var o = (object)data;
                List<string> paramNames = GetParamNames(o);
                if (autoId)
        ...
        public Task<T> ExecuteScalarAsync<T>(string sql, dynamic param = null)
        {
            return connection.ExecuteScalarAsync<T>(sql, param as object, transaction, this.commandTimeout);
        }
