...
namespace YF.Utility.Logging {
    public class IQCFileAppender : RollingFileAppender {
        /// <summary>
            ...
            lock (this) {
                ...
                for (int i = 1; !fileOpened && i <= Retries; i++) {
                    try {
                        ...
                        LogLog.Error(typeof(IQCFileAppender), string.Format("IQCFileAppender: Failed to open [{0}]. Attempting [{1}-{2}] instead.", fileName, fileName, newSuffix));
                    }
                }
